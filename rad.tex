...
U ovoj glavi opisana je statička analiza softvera, kao jedan od načina ispitivanja ispravnosti programa, u delu \ref{sec:deo1}. U delu \ref{sec:deo2} opisan je sistem za statičko utvrđivanje ispravnosti softvera LAV. U delu \ref{sec:deo3} navedena je motivacija i značaj automatskog generisanja test primera. U delovima \ref{sec:deo4} i \ref{sec:deo5} opisana je integracija koda u postojeće klase i module kao i implementacija novih klasa sa ciljem automatskog generisanja test primera u okviru alata LAV.

%implementacija modula za automatsko generisanje test primera integrisanog u sistem LAV.
...
Automatsko generisanje test primera upotrebom rešavača Z3 ostvareno je nadogradnjom postojećih klasa i modula alata LAV. Proces automatskog generisanja test primera primenom tehnika statičke analize i rešavača Z3 ima dva osnovna dela. Prvi deo predstavlja modelovanje uslova ispravnosti programa formulama izabrane teorije rešavača i ostvarena je dopunom klase \texttt{z3-instance}. Ponašanje programa upotrebom rešavača Z3 opisuje se formulom teorije linearne aritmetike (klasa \texttt{la-expression-z3}) ili teorije bitvektora (klasa \texttt{bv-expression-z3}).



%Arhitektura modula za automatsko generisanje test primera dizajnirana je tako da ispunjava zahteve sistema LAV. 
%Modul  za automatsko generisanje test primera primenom tehnika statičke analize i rešavača Z3 ima dva osnovna dela. 
%Prvi deo predstavlja modelovanje uslova ispravnosti programa formulama izabrane teorije rešavača (klasa \texttt{z3-instance}). Ponašanje programa upotrebom rešavača Z3 opisuje se formulom teorije linearne aritmetike (klasa \texttt{la-expression-z3}) ili teorije bitvektora (klasa \texttt{bv-expression-z3}).

%U okviru ovih teorija, dodata je podrška za teoriju nizova (klasa \texttt{AbstractArray}). 
...
%Modul je implementiran u programskom jeziku C++ zbog kompatibilnosti sa sistemom LAV. 
...
%Treba objasniti da generisanje modela ne zahteva anotacije, jer nije bitan redosled nabrajanja promenljivih, dok generisanje testa zahteva odredjen redosled, a automatsko odredjivanje redosleda moze da bude pravac daljeg rada jer to nije trivijalan problem.
