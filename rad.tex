...
% ------------------------------------------------------------------------------
\chapter{Uvod}
%Intenzivan razvoj informatike i racunarstva u poslednjim 		%decenijama uveo je softver u sve oblasti zivota. 			Razvijeni %su brojni softverski proizvodi sa vrlo razlicitim 	namenama.  Softver %je postao neophodan u svim 			oblastima drustva:  privredi, %obrazovanju, zdrastvu, 			medijima i komunikacijama, %poslovanju, politici, itd.
...
%U kontekstu softvera, zbog velikog prostora stanja koji je potrebno %sistematski pretražiti, proveravanje modela se najčešće koristi za %pronalaženje grešaka u sistemu, kao dopuna testiranju programa, a ne %kao metod potpune verifikacije programa.

%U okviru automatizovane provere ispravnosti izdvajaju se formalne %metode verifikacije u kojima se uslovi ispravnosti softvera iskazuju u %terminima matematičkih tvrđenja na striktno definisanom formalnom %jeziku izabrane matematičke teorije.
...
%Pored ovog fundamentalog ograničenja, postoje i praktična ograničenja %koja se odnose na postojanje konačnih resursa: konačne memorije i %ograničenog vremena za analizu ispravnosti. To znači da, čak i za %programe koji imaju konačan prostor mogućih stanja i za koje je %problem ispitivanja ispravnosti teorijski odlučiv, ovaj prostor može %da bude prevelik i da, usled ograničenosti resursa, teorijsko rešenje %problema 
%nije praktično upotrebljivo.



%Analiza se može izvršiti pregledom koda od strane programera, može %biti poluautomatizovana (programer uz pomoć specijalizovanih alata %proverava ispravnost softvera) ili može biti potpuno automatizovana. %Pregledanje koda najčešće nije dovoljno pouzdan način otkrivanja %grešaka. Poluautomatski pristupi mogu biti vremenski zahtevni i %iscpljujući po programera. Za automatske pristupe postoji važno %teorijsko ograničenje: neodlučivost. To znači da nije moguće %konstruisati algoritam koji u konačnom vremenu automatski utvrđuje %ispravnost proizvoljnog programa potpuno precizno. Pored ovog %fundamentalnog ograničenja, postoje i praktična ograničenja koja se %odnose na postojanje konačnih resursa: konačne memorije i ograničenog %vremena za analizu ispravnosti. To znači da, čak i za programe koji %imaju konačan prostor mogućih stanja i za koje je problem ispitivanja %ispravnosti teorijski odlučiv, ovaj prostor može da bude prevelik i %da, usled ograničenosti resursa, teorijsko rešenje problema nije %praktično upotrebljivo. Statičke tehnike proveravanja ispravnosti %softvera mogu da koriste različite teorije za modelovanje ponašanja %programa.  
%Zbog toga se u procesu razvoja softvera posebna pažnja poklanja %verifikaciji softvera, tj. ispitivanju ispravnosti softvera.


%Neispravan softer kosta svetsku ekonomiju milijarde 		dolara %godisnje. Medjutim, sustinska vaznost ispravnosti 		softvera ne %meri se novcem jer neispravan softver moze da 	uzrokuje materijalno %nesagledive posledice. To cini pitanje 	ispitivanja ispravnosti %softvera,  odnosno verifikacije softvera 	posebno vaznim. Paralelno %sa razvojom novih tehnologija, a 	u cilju pruzanja novih kvaliteta i %udobnosti u radu, programi 	koji pruzaju korisnicima, razlicite %usluge, kao i programi koji 	kontrolisu uredjaje prisutne svuda oko %nas, postaju sve 		slozeniji. Slozenost programa neminovno povlaci %i veci 		prostor za pravljenje gresaka. Ove greske, u zavisnosti %od 	namene samog programa,  mogu da budu neprijatne po 		%pojedinca, ali mogu da imaju i fatalne posledice po zivote i 	%zdravlje ljudi.  Zbog toga se u procesu razvoja softvera, 		posebna %paznja poklanja verifikaciji softvera.

%Složenost softverskih sistema neminovno povlači i veći prostor za %pravljenje grešaka. U zavisnosti od namene, greške mogu dovesti do %neprijatnosti ali i izazvati katastrofalne posledice po živote ljudi.
%Kako bismo izbegli ovakve situacije, neophodno je detaljno i precizno %utvrditi ispravnost razvijenog softvera.
%Greske u softveru mogu imati katastrofalne posledice, na primer %otkazivanje medicinskog uredjaja u toku operacije ili otkazivanje %automatskog pilota u toku leta. Kako bismo preduhitrili ovakve %situacije, potrebno je detaljno i precizno utvrditi ispravnost %razvijenog softvera.

%Prisutan svuda oko nas, softver predstavlja važan deo svakodnevnog %života. Softver nam pruža mogućnost udobnog rukovanja različitim %uređajima, olakšava nam i pojednostavljuje različite životne %situacije.

%Razvoj i primena novih tehnologija počinju sve više da utiču na %društvo. Računari su prisutni svuda oko nas i pružaju nam mnoge %pogodnosti i olakšice kako u neobaveznim tako i u poslovnim %aktivnostima. Institucije poput banaka, zdravstva, prosvete i drugih %teže da automatizuju svoje procese korišćenjem različitih softverskih %alata.
\subsection{Motivacija}
%U prethodne dve decenije se dosta toga promenilo u pogledu performansi računara. Današnji računari su sve češće višeprocesorski i višejezgarni. Ovaj porast performansi omogućio je, između ostalog, brže i iscrpnije testiranje.  \par
%UOPSTENO O TESTIRANJU, MOJE TESTIRANJE, 
%O kvalitetu softvera mora se voditi računa tokom celokupnog procesa razvoja. 
%Kvalitet softvera u velikoj meri zavisi od dobrog testiranja. Analiza koda je jedna od metoda testiranja koja se primenjuje prva. OVDE TREBA DA JE NAJPOUZDANIJA ALI DA IMA NEDOSTATAK. 


%AUTOMATIZOVANO STATICKO PROVERAVANJE ISPRAVNOSTI SOFTVERA pomenuti %staticku analizu
%Automatizovano statičko utvrđivanje ispravnosti softvera

%automatsko generisanje test primera kojima se proverava ispravnost %programa bitno da se taj proces automatizuje bla bla 


%Statičke tehnike verifikacije softvera mogu da koriste različite %teorije za modelovanje ponašanja programa.
...
%U okviru automatizovane provere ispravnosti softvera, ponašanje programa može se modelovati matematičkim tvrđenjima definisanim formalnim jezikom izabrane matematičke teorije.

%U okviru automatizovane provere ispravnosti izdvajaju se formalne metode verifikacije u kojima se uslovi ispravnosti softvera iskazuju u terminima matematičkih tvrđenja na striktno definisanom formalnom jeziku izabrane matematičke teorije. 
%Da bi proverio da li neka naredba može da dovede do greške u programu, sistem gradi formule koje predstavljaju uslove ispravnosti naredbi i koje pripadaju izabranoj kombinaciji predloženih teorija.
%Uslovi ispravnosti se proveravaju odgovarajućim rešavačem na osnovu čijih rezultata se generiše izveštaj o statusu naredbi u programu.
...
Nastavak rada organizovan je na sledeći način.
% ------------------------------------------------------------------------------
...
%Testiranjem se postiže verifikacija softvera. Dana, principi verifikacije u cilju %testiranja softvera koriste različite tehnologije i alate kao što je proveravanje %modela, SAT rešavači i provera ispunjenosti raznih ograničenja. Sve ovo se radi sa %ciljem automatizacije generisanje test slučajeva. Mnogi od automatskih načina %testiranja se zasnivaju na teorijskim vezama između verifikacije i testiranja. 
%Svakako glavna primena automatskog generisanja test slučajeve je u polju verifikacije %softvera.
%Da bi se ispitala ispravnost softvera, najpre je potrebno precizno definisati njegovo %željeno ponašanje. Željeno ponašanje softvera zadaje se specifikacijom. Specifikacija %daje opis šta softver treba da radi. Verifikacija je postupak dokazivanja da je %program ispravan, odnosno da zadovoljava specifikaciju. 
