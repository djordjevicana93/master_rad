...
\par
Ulazni format rešavača Z3 je definisan SMT-LIB 2.0 standardom REF. Standard definiše jezik logičkih formula čija se zadovoljivost proverava u odnosu na neku teoriju. Cilj standarda je obezbeđivanje zajedničkog jezika za sve SMT rešavače. 
\par
...
U okviru celobrojne linearne aritmetike, podržani funkcijski simboli su +, -, $\sim$ (unarna negacija), * gde je drugi operand konstanta. Pored ovih podržane su operacije div i mod, uz uslov da je drugi operand konstanta različita od 0. Nad funkcijskim simbolima, čiji su specijalni slučajevi konstante mogu se primenjivati relacijski operatori <, <=, > i >=. 
\par
U okviru realne linearne aritmetike, podržani funkcijski simboli su +, -, $\sim$ (unarna negacija), * pri čemu je drugi operand konstanta. Nad funkcijskim simbolima, čiji su specijalni slučajevi konstante mogu se primenjivati relacijski operatori <, <=, > i >=. 
\par
...
Odavde zaključujemo da je polazna formula validna.
\\ 
\hspace{9.5cm}
...
Rešavač Z3 obezbeđuje interfejse za direktnu podršku sa programskim jezicima C, C++ i Python. U nastavku će biti navedeni primeri korišćenja C++ API-ja sa komunikaciju sa rešavačem Z3.
\begin{primer} Primer demonstrira dokazivanje De Morganovog zakona korišćenjem C++ API-ja. \\ \\
\begin{minipage}[b]{0.5\textwidth}
\begin{verbatim}
/**
   Dokazujemo zakon koriscenjem formule:
   not(x and y) <-> (not x) or ( not y) }
*/
void demorgan() {
    //deklarisanje konteksta koji se prosledjuje resavacu
    context c;
    
	    //deklarisanje konstanti i izraza
    expr x = c.bool_const("x");
    expr y = c.bool_const("y");
    expr e = (!(x && y)) == (!x || !y);
    
    solver s(c);
    //dodavanje negacije izraza kao ogranicenja
    s.add(!e);
...
\begin{primer} Primer ilustruje traženje interpretacija celobrojnih konstanti x i y, pri čemu moraju biti ispunjene nejednakosti x >= 1 i y < x+3. U okviru modela, pronalaze se interpretacije funkcijskih simbola. U ovom slučaju to su interpretacije konstanti, kao funkcijskih simbola arnosti 0.\\
\begin{minipage}[b]{0.5\textwidth}
\begin{verbatim}
void find_model_example1() {
	//deklarisanje konteksta i celobrojnih konstanti x i y
    context c;
    expr x = c.int_const("x");
    expr y = c.int_const("y");
    solver s(c);
	//dodavanje ogranicenja resavacu
    s.add(x >= 1);
    s.add(y < x + 3);
    model m = s.get_model();
    std::cout << m << "\n";
    for (unsigned i = 0; i < m.size(); i++) {
        func_decl v = m[i];
        //odredjivanje interpretacija funkcijskih simbola arnosti 0
        assert(v.arity() == 0); 
